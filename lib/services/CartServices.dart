...
class CartServices {
  ...
  static addNewCart(item) async {
    item = CartServices.formatCartData(item);
    String? cartList = await Storage.getString('cartListNew');
    if (cartList != null) {
      List cartListData = json.decode(cartList);
      //判断购物车有没有当前数据
      bool hasData = cartListData.any((value) {
      ...
      if (hasData) {
        ...
        await Storage.setString('cartListNew', json.encode(cartListData));
      } else {
        cartListData.add(item);
        await Storage.setString('cartListNew', json.encode(cartListData));
      }
      ...
      List tempList = [];
      tempList.add(item);
      await Storage.setString('cartListNew', json.encode(tempList));
    }
  }

  static removeNewCart() async{
    var s = await Storage.getString('cartListNew');
    await Storage.remove('cartListNew');
  }
